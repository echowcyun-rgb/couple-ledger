...
> 更新时间：2026-07-01（第七轮：头像性别分组修正 + PWA 图片缓存 v30）

## 项目信息
- 路径：~/Documents/记账app/-pwav1-移植版
- 分支：main（优化 12–18 已推送并部署）
- 生产地址：https://couple-ledger.vercel.app（当前线上为优化 12–18 版本）
...
- 线上最新部署：优化 12–18 已部署，状态 READY
- 本地待推送：头像性别分组修正 + PWA 图片缓存 v30（已本地验证 build/test）
...
- **修复**（2026-07-01）：
  - `CACHE_NAME` 升级至 `couple-ledger-v30`
  - `LOCAL_ASSETS` 扩展 tabbar 10 张、sys-av 10 张、avatars 2 张
  - 保持不缓存 `/_next/*` 与 `/api/*`

## 测试要求
- ✅ `npm run build` + `npm test`（35 passed，2026-07-01 头像/PWA 修复后验证）
- 运行 Supabase 真实集成测试（或至少验证 build/dev 无错）
- 验证成员编辑页性别切换后头像分组正确
- 验证生产构建后 SW 缓存了新增图片资源

## 最近 commit 记录
```
（待提交）fix: 头像性别分组修正 + PWA 图片缓存 v30
...
- `npm run build` + `npm test`（35 passed）已通过（2026-07-01 头像/PWA 修复后验证）
