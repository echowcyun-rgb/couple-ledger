...
> 更新时间：2026-07-01（第十轮：手机端卡加载中 + RoomSetup 进入流程修复 + SW v32）
...
## 已完成优化（20：手机端卡加载中 + RoomSetup 进入流程修复）

### 问题 1：手机端重新打开网页卡在「加载中」❌ → 已修复
- **根因**：`sw.js v31` 缓存了首页 HTML shell（`LOCAL_ASSETS` 含 `"./"`），但 `/_next/*` JS 不缓存。手机重新打开时 SW 返回旧 HTML shell，浏览器再拉新 `_next` JS，两者 hash 不匹配 → React hydration 失败 → `useLedger` 的 `useEffect` 不执行 → `hydrated` 永远 `false` → 卡在「加载中」。
- **修复**（`public/sw.js` v32）：
  - `LOCAL_ASSETS` 移除 `"./"`，不再缓存首页 HTML
  - fetch 阶段对 `navigate` / `document` / `/` 一律 `fetch(event.request)` 走网络，确保 HTML 与 JS 同版本
  - 删除 fetch 回退中 `caches.match("./")` 逻辑

### 问题 2：每次重新进入网页都跳转创建房间界面 ❌ → 已修复
- **根因**：`app/page.tsx` 本次改动引入 `setupReady` 机制（解决 SSR 读不到 localStorage 的问题），但误写成 hydration 后无条件 `setShowSetup(true)`，导致老用户每次都被跳到 RoomSetup。
- **修复**（`app/page.tsx`）：
  - 保留 `setupReady` 机制（SSR 安全）
  - hydration 完成后**始终显示 RoomSetup**：新用户创建/加入，老用户点「进入 #xxxx 房间」按钮进首页
  - 老用户点击「进入房间」→ `onDone(existingRoom, false)` → `enterRoom(roomId, {fresh:false})` → 复用本地已有记录立即进首页 → 后台 `syncFromCloud()` 拉取云端最新数据

### 问题 3：RoomSetup UI 重构
- **修复**（`components/modals/RoomSetup.tsx`）：
  - `mode` 状态从 `"choose"` 改为 `"create"`（默认即创建页，无需多余状态）
  - 创建成功弹窗关闭按钮 `setMode("choose")` → `setCreatedRoom("")`
  - 加入弹窗取消/遮罩 `setMode("choose")` → `setMode("create")`
  - 创建中 loading 仅在 `loading` 时显示，无 loading 不渲染多余 DOM
  - 主操作区始终显示三按钮（创建/进入/加入），弹窗仅在 create/join 时叠加
...
- ✅ 提交并推送 startDate 随房间云同步（commit `58fb4d4`）
- ✅ 修复手机端卡加载中（SW v32）+ RoomSetup 进入流程 + UI 重构（本次）
...
- 有已存房间号时每次进入显示 RoomSetup，老用户点「进入 #xxxx 房间」按钮进首页（复用本地数据 + 后台云同步）；无房间时显示创建/加入
...
- 当前 SW 缓存版本：`couple-ledger-v32`；生产 PWA 更新后若异常可 Unregister SW 再硬刷新
