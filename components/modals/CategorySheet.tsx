  ...
  return (
    <>
      ...
      >
        ...
        <div className="goal-add cat-add-form">
          <div className="ga-title">新增分类</div>
          <div className="cat-type-seg">
            {([["out", "支出"], ["in", "收入"], ["save", "存钱"]] as [TxType, string][]).map(([k, l]) => (
              <button key={k} type="button" className={`seg-btn ${newCatType === k ? "on" : ""}`} onClick={() => setNewCatType(k)}>
                {l}
              </button>
            ))}
          </div>
          <input
            className="ga-input cat-name-input"
            type="text"
            placeholder="分类名称，如：奶茶"
            value={newCatLabel}
            onChange={(e) => setNewCatLabel(e.target.value)}
            maxLength={12}
            autoComplete="off"
          />
          <button type="button" className="px-btn ga-btn" disabled={adding || !newCatLabel.trim()} onClick={handleAdd}>
            ＋ 添加分类
          </button>
        </div>
